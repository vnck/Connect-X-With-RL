...
def policyIterSP(game,nnet,iters,eps,n_sims=30):
...
class ExampleDataset:
    ...
    def __getitem__(self,index):
        """
        Note that connect4 is horizontal flip invariant
        """
        return transforms.RandomHorizontalFlip(p=0.5)(torch.tensor(self.examples[index][0],dtype=torch.float32)),torch.tensor(self.examples[index][1],dtype=torch.float32),torch.tensor(self.examples[index][2],dtype=torch.float32)
...
class NNET:
    ...
    """
    def __init__(self,nnet,device,epoch=10):
        self.nn = nnet
        self.epoch = epoch
        self.device = device
...
def trainNNet(examples,nnet):
    device = torch.device( "cuda" if torch.cuda.is_available() else "cpu" )
    nnet = NNET(nnet,device)
    nnet.train(examples)
    return nnet.nn


if __name__ == "__main__":
    final_net = policyIterSP(game=Game(),nnet=Connect4Net(),iters=100,eps=100,n_sims=25)
